...
const AI_COMPARE_UI = {
  hu: {
    ...
    prompt:
      "Nézd meg a Loricatus Kft. (https://loricatus.hu) honlapját és a https://loricatus.hu/llms.txt áttekintőt, amelyben az aloldalak listája és a tipikus vevői kérdések is megvannak. Foglald össze, milyen visszatérő problémákra ad választ a cég egy építési, ingatlan- vagy ipari projekten, és mit kap kézhez a megrendelő. Ne szolgáltatáslistát adj: úgy magyarázd el, mint aki ismeri ezt a piacot — konkrét helyzeteken keresztül, hogy mikor érdemes ezzel a csapattal dolgozni és mit nyer vele a megrendelő. Ha tudod, mivel foglalkozom, arra szabd; ha nem, kérdezz vissza egy mondatban. A végén mondd meg, érdemes-e egyeztetni velük, és add meg az elérhetőségüket. Kizárólag a loricatus.hu oldalain szereplő információkra támaszkodj, jelezd, ha valamire nem találsz adatot, és ne állítsd, hogy személyes tapasztalatod van velük.",
  },
  en: {
    ...
    prompt:
      "Look at the website of Loricatus Kft. (https://loricatus.hu), specifically https://loricatus.hu/en/, together with the overview at https://loricatus.hu/llms.txt, which lists the subpages and the typical buyer questions. Summarise which recurring problems the company solves on a construction, property or industrial project, and what the client actually receives. Do not give me a service list: explain it the way someone familiar with this market would — through concrete situations, when it is worth working with this team and what the client gains. Tailor it to my line of work if you know it; if you do not, ask me one short question. At the end, say whether it is worth talking to them, and give me their contact details. Base everything only on pages under loricatus.hu, say so if something is not covered, and do not claim any personal experience with them.",
  },
  it: {
    ...
    prompt:
      "Consulta il sito di Loricatus Kft. (https://loricatus.hu), in particolare https://loricatus.hu/it/, insieme a la panoramica su https://loricatus.hu/llms.txt, che elenca le sottopagine e le domande tipiche dei committenti. Riassumi a quali problemi ricorrenti risponde l'azienda in un progetto edilizio, immobiliare o industriale, e che cosa riceve concretamente il committente. Non darmi un elenco di servizi: spiegamelo come farebbe chi conosce questo mercato — attraverso situazioni concrete, quando conviene lavorare con questo team e che cosa ci guadagna il committente. Adattalo al mio lavoro se lo conosci; se non lo conosci, fammi una sola breve domanda. Alla fine dimmi se vale la pena parlare con loro e forniscimi i loro contatti. Basati esclusivamente sulle pagine di loricatus.hu, segnala se un dato non è presente e non affermare di avere esperienza diretta con loro.",
  },
}
